...
# 返回灰度掩码
def get_mask_ch1(gary_im, quantile_value1=30, quantile_value2=95):
    '''
    输入输出皆为单通道灰度图像
    '''
    # 找到两个分位点
    quantile1 = np.percentile(gary_im, quantile_value1)
    quantile2 = np.percentile(gary_im, quantile_value2)

    # 创建掩码：小于分位点的像素值为1，大于等于分位点的像素值为0
    l_mask = np.where(gary_im < quantile1, 1, 0)
    m_mask = np.where((gary_im >= quantile1) & (gary_im < quantile2), 1, 0)
    h_mask = np.where(gary_im >= quantile2, 1, 0)

    return quantile1, quantile2, l_mask, m_mask, h_mask

# 返回彩色掩码
def get_mask_ch3(color_im):
    '''
    输入输出皆为三通道彩色图像,其中掩码在三个通道一致，即单通道权重复制为三通道
    '''
    gary_im = to_gary(color_im)

    quantile1, quantile2, l_mask, m_mask, h_mask = get_mask_ch1(gary_im)

    gray_masks = [l_mask, m_mask, h_mask]
    color_masks = []

    for mask in gray_masks:
        # print(mask.shape)
        mask = np.expand_dims(mask, axis=2)
        color_masks.append(np.concatenate((mask, mask, mask), axis=-1))

    return quantile1, quantile2, color_masks[0], color_masks[1], color_masks[2]
