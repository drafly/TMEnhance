...
def main():
    ...
    input_folder = '/root/autodl-tmp/dataset/HDRTV1k/train_set/train_sdr'
    save_folder = '/root/autodl-tmp/dataset/HDRTV1k/train_set/train_sdr_sub'
    # input_folder = '/data/dataset/xiaozhou/HDRTV1k/results/adaptive_global_color_mapping/test_set'
    # save_folder = '/data/dataset/xiaozhou/HDRTV1k/results/adaptive_global_color_mapping/test_set_sub'

    n_thread = 20
    crop_sz = 480   # crop size
    step = 480  # crop stride
    thres_sz = 48
    compression_level = 0  # 3 is the default value in cv2
    # CV_IMWRITE_PNG_COMPRESSION from 0 to 9. A higher value means a smaller size and longer
    # compression time. If read raw images during training, use 0 for faster IO speed.
    # 设置 .png 格式图片的压缩比，取值为 0-9（默认值 3），数值越大则无损压缩比越大，0表示不压缩直接储存。默认值为1（最佳速度设置）
    ...
    for path in img_list:
        pool.apply_async(worker,
            ...
            callback=update)
    pool.close()    # 关闭pool, 使其不再接收新的（主进程）任务
    pool.join()     # 主进程阻塞后，让子进程继续运行完成，子进程运行完成后，再把主进程全部关掉
